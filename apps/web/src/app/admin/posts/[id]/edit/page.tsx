...
export default function EditPostPage() {
  ...
  useEffect(() => {
    ...
    async function fetchPost() {
      try {
        // Fetch all posts to find by ID (posts endpoint uses slug for single fetch)
    ...
    return (
      <div className="min-h-screen bg-light flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/admin/posts" className="btn-primary">Back to Posts</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-light">
      <div className="bg-white border-b border-light">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <p className="text-primary/60 mt-1">{post.title}</p>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="bg-white rounded-xl border border-light p-6 space-y-5">
            ...
            <div>
              ...
              <input id="title" type="text" value={title} onChange={(e) => setTitle(e.target.value)} className="input-field" required />
            </div>
            ...
            <div>
              ...
              <textarea id="excerpt" value={excerpt} onChange={(e) => setExcerpt(e.target.value)} rows={2} className="input-field resize-y" />
            </div>

            <div>
              <label htmlFor="content" className="block text-sm font-medium text-primary mb-1.5">Content *</label>
              <textarea id="content" value={content} onChange={(e) => setContent(e.target.value)} rows={12} className="input-field resize-y font-mono text-sm" required />
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-light p-6 space-y-5">
            ...
            <div>
              ...
              <input id="featuredImage" type="url" value={featuredImage} onChange={(e) => setFeaturedImage(e.target.value)} className="input-field" />
            </div>
          </div>
